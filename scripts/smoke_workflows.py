#!/usr/bin/env python3
"""Browser workflow checks for the shipped RHW route manifest."""
...
def test_v402(cdp, workspace, node):
    if (workspace, node) == ("command", "inventory"):
        states = base.ev(cdp, "(()=>{const f=(verified,stale)=>{window.hasVerifiedTelemetry=()=>verified;dataIsStale=stale;lastSyncError='';lastLoaded=verified?new Date():null;RHWV4.command.updateOverview();RHWV4.accessibility.sync();return [v40OverviewTelemetryState.textContent.trim(),v40OverviewTelemetryState.dataset.state]};return{live:f(true,false),cache:f(true,true),offline:f(false,false)}})()")
        ...
        labels = base.ev(cdp, "(()=>{RHWV4.accessibility.sync();return [...document.querySelectorAll('[data-material-price]')].map(x=>x.getAttribute('aria-label')||'')})()")
    ...
    for width in MOBILE_WIDTHS:
        ...
        try:
            result = base.ev(cdp, """(()=>{
              ...
              RHWV4.priceProfiles.ensureProfilePanel();
...
def test_pr4_pwa(cdp, workspace, node):
    ...
    try:
        result = base.ev(cdp, """(async()=>{
          RHWV4.toolsNavigation.openTools();
...
def test_pr6_discovery_status(cdp, workspace, node):
    ...
    try:
        base.ev(cdp, "RHWV4.toolsNavigation.openTool('data')")
        result = base.ev(cdp, """(()=>{
        ...
        base.ev(cdp, "RHWV4.diagnostics.close()")
        cdp.call("Emulation.clearDeviceMetricsOverride")
...
def test_pr7_diagnostics(cdp, workspace, node):
    ...
    try:
        result = base.ev(cdp, """(async()=>{
          ...
          app.toolsNavigation.openTools();
          const opener=document.querySelector('[data-rhw-tool="system"]');
          const buttonHeight=opener.getBoundingClientRect().height;
          opener.click();
          ...
          const snapshot={
            api:!!api,button:!!opener,buttonHeight,
...
def test_pr11_full_audit(cdp, workspace, node):
    ...
    try:
        result = base.ev(cdp, """(async()=>{
          ...
          app.navigate('command','inventory',{replace:true});
          return{
    ...
    if result.get("routeStatus") != "9 MOUNTED" or result.get("routeTone") != "good" or not result.get("privacy"):
