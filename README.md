...
`COMMAND / SHIPYARD` shows selectable stock cards for **Dunkirk**, **Invincible** and the **Archon Modular Miner**. Below them, **Build Requirements** uses the selected ship's Calculator recipe to show material coverage, the bottleneck and shortages for the next ship beyond the covered quantity. Crew and blueprints are separate, reusable prerequisites; unknown or cached stock is labelled. **PRICE 1 SHIP** opens that exact recipe in the Calculator. The last ship selection is remembered and travels with app-settings backups.
