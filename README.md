...
Shared colour, spacing and type tokens live in `css/01-core.css`; component rules live in the listed source stylesheets. `css/35-app-interface-cleanup.css` owns the current interface. Editing existing rules is preferred to adding another override sheet. Font files and upstream licenses are in `assets/fonts`.

`RHWV4.onUiUpdate` coalesces status work after telemetry and user changes. Hidden pages pause scheduled telemetry and clock updates, and refresh once due when visible again. Only visible clock surfaces tick. `js/14a-legacy-archive.js` preserves old order backups without loading the former board. Retired editor source remains historical reference and is excluded from the production manifest and offline shell.
