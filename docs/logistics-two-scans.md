...
The material scan replaces the fixed Lisheen/Shelton cards. Prototype Components appear only in Industrial Materials.
...
Narrow seller cards place the full base name above the stock and price columns.
Number columns grow with their content; input stocks share a consistent trailing
alignment. Mobile offer disclosure stays open across refreshes and sorting for
each commodity. A focused disclosure keeps keyboard focus on that channel after
refresh, including when the remaining offers no longer need a disclosure.
